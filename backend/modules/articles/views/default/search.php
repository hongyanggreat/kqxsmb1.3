      ...
      <div class="span7 offset2">
        <div class="widget-box">
          <div class="widget-title"> 
            <span class="icon"><a class="" href="<?= $baseUrl.$module ?>" title="Danh sách dịch vụ" style="padding:0 2px;"><i class="icon icon icon-home" style="color:#fff"></i></a></span>
          ...
          <div class="widget-content nopadding">
            ...
            <?= $form->field($model, 'TITLE', ['template' => '{label} <div class="controls">{input}{error}{hint}</div>','options' => ['class' => 'control-group']])->textInput(['class'=>'span11','placeholder'=>'Tên dịch vụ']) ?>
            ...
            <?php
              ...
              // Mượn POSITION làm sắp xếp theo trường 
               $orderBy= [
                'DESC' =>'Giảm dần',
                'ASC'  =>'Tăng dần',
              ]; 
              $orderByRecord =  $form->field($model, 'POSITION',['template' => '{input}{error}','options' => ['class' => '']])->dropDownList($orderBy,['options'=>[$model->POSITION =>['Selected'=>true]],'class'=>'span4'])->label('Sắp xếp'); ?>
            ...
            <?php 
              ...
              $dataField= [
                  'ID'           =>'Theo ID dịch vụ',
                  'NAME_SERVICE' =>'Theo tên dịch vụ',
                  'STATUS'       =>'Theo trạng thái',
                  'CREATED_AT'   =>'Theo ngày tạo',
                  
              ];
              echo $orderRecode  = $form->field($model, 'DESCRIPTION',['template' => '{label} <div class="controls">{input}'.$orderByRecord.'{error}{hint}</div>','options' => ['class' => 'control-group']])->dropDownList($dataField,['options'=>[$model->DESCRIPTION =>['Selected'=>true]],'class'=>'span4'])->label('Sắp xếp'); ?>
            ...
            <?php  
              ...
              $limitRecode  = $form->field($model, 'UPDATED_AT',['template' => '{label} <div class="controls">{input}'.$textLimit.'{error}{hint}</div>','options' => ['class' => 'control-group']])->dropDownList($dataModule,['options'=>[$model->UPDATED_AT =>['Selected'=>true]],'class'=>'span4'])->label('Giới hạn bản ghi'); ?>
